...
extension CityListView {
    ...
    func masteryStats(for city: CityLocation.City) -> (tier: String, color: Color, progress: Double) {
        let visitedCount = city.buildings.filter { GlobalProgressManager.shared.isVisited($0.id) }.count
        let total = city.buildings.count
        guard total > 0 else { return ("Tourist", .blue, 0) }
        
        let percentage = Double(visitedCount) / Double(total)
        
        func accessibilityColor(_ color: Color) -> Color {
            isHighContrast ? .primary : color
        }
        
        switch percentage {
        case 0..<0.25: return ("Tourist", accessibilityColor(.blue), percentage)
        case 0.25..<0.50: return ("Explorer", accessibilityColor(.green), percentage)
        case 0.50..<0.75: return ("Insider", accessibilityColor(.purple), percentage)
        case 0.75..<1.0: return ("Legend", accessibilityColor(Color.adventureOrange), percentage)
        default: return ("Master", accessibilityColor(Color.adventureOrange), 1.0)
        }
    }
